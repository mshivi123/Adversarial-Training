...
learning_rate_pre = tf.train.exponential_decay(self.starter_learning_rate,global_step_pre,200, 0.96, staircase=True)
...
self.pretrain_manager_grad, _ = tf.clip_by_global_norm(tf.gradients(self.pretrain_goal_loss, self.manager_params), self.grad_clip)
            self.pretrain_manager_updates = pretrain_manager_opt.apply_gradients(list(zip(self.pretrain_manager_grad, self.manager_params)),
                                                                                 global_step=global_step_pre)
        # self.real_goal_array = self.real_goal_array.stack()
...
def _g_recurrence_2(i, x_t,gen_x,h_tm1,h_tm1_manager,last_goal,real_goal):
           ...
           with tf.variable_scope(self.scope):
               feature = self.FeatureExtractor_unit(cur_sen,self.drop_out)
           h_t_Worker = self.g_worker_recurrent_unit(x_t, h_tm1) 
           o_t_Worker = self.g_worker_output_unit(h_t_Worker) 
...
real_sub_goal = tf.add(last_goal,sub_goal)
w_g = tf.matmul(real_goal,self.g_change)   
w_g = tf.nn.l2_normalize(w_g, 1)
...
log_prob = tf.log(tf.nn.softmax(x_logits))
            ...
            with tf.control_dependencies([cur_sen]):
                gen_x = gen_x.write(i-1, next_token) 
        ...
        gen_for_reward = gen_for_reward.stack() 

        gen_for_reward = tf.transpose(gen_for_reward, perm=[1, 0])  
